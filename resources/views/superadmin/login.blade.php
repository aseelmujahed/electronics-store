<x-app-layout>
    <div class="min-h-screen flex items-center justify-center bg-gradient-to-tr from-blue-50 via-purple-50 to-blue-100">
        <div class="max-w-md w-full p-8 bg-white rounded-2xl shadow-xl border border-blue-100 flex flex-col items-center">
            <h2 class="text-2xl font-bold mb-2 text-blue-700 text-center">Super Admin Login</h2>
            <p class="text-gray-500 mb-6 text-center text-sm">Enter your credentials to access the super admin dashboard.</p>
            <form method="POST" action="{{ route('superadmin.login') }}" class="w-full">
                @csrf
                @if($errors->any())
